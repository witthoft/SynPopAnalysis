% k-means clustering of face similarity data.
...
tree = linkage(dists, clstdef);

figure(n); n=n+1;
H = dendrogram(tree,0,'colorthreshold',1.95,'Labels',num2str(ids(:,1)));
...
for ctr = 1:length(labels)
    pic = char([strtrim(strcat(labels(ctr),'.jpg'))
        ])
...
xlabel(char('faces',['cophenetic correlation coefficient = ' num2str(cr,2)]),'FontSize',24);
...
T=cluster(tree,'Cutoff',1.95,'Criterion','Distance');
for i=1:max(T)
...
% or you can assume 6 groups which might not works as well


% now try clustering using k-means algorithm.  difference is k-means tries
% to minimize the sum squared distance within a group. unlike linkage,
% number of groups has to be spedified beforehand.  