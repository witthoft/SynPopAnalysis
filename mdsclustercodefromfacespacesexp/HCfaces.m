...
% we also need colors for the labels so start off with a list of colors
% where there are identity x view rows
% color index is arbitrary but we can go with
...
if exist('altnames') 
    ...
    for i=1:length(new_labels)
        ...
        lcmap = [lcmap; nlmap(perm(i), :)];  %get the associated font color for each label
    end
