function  f_visualizeEagDBsubset(rgb_data)
...
subplot(1,2,1);
imagesc(rgb_data(:,fqorder,:));
ylabel('SUBJECTS');
xlabel('LETTERS');
set(gca,'XTick',[1:26],'XTickLabel',letters(fqorder), 'TickDir','out', 'YDir','normal');
box off;
title('fq in english');
...
subplot(1,2,2);
imagesc(rgb_data(:,fqfrst,:));
ylabel('SUBJECTS');
xlabel('LETTERS');
set(gca,'XTick',[1:26],'XTickLabel',letters(fqfrst), 'TickDir','out', 'YDir','normal');
box off;
...
% clrs = squeeze(rgb_data(:,i,:));
% scatter3(rgb_data(:,i,1),rgb_data(:,i,2),rgb_data(:,i,3),20,clrs);
